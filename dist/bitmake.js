...
function makeBuildConfig(gconfig, config) {
    ...
    rootConfig.binaryRoot = rootConfig.binaryRoot || _utils_Path__WEBPACK_IMPORTED_MODULE_5__.Path.join(gconfig.workDir, "build");
...
class BuildContext {
    ...
    async doExtractArchive(gconfig, environment, config, settings) {
        if (!config.sourceUrl)
    ...
    async getUserConfig(options) {
        let configPath;
        if (options.env.config) {
            configPath = _utils_Path__WEBPACK_IMPORTED_MODULE_5__.Path.isAbsolute(options.env.config) ? options.env.config : _utils_Path__WEBPACK_IMPORTED_MODULE_5__.Path.resolve(options.workDir, options.env.config);
            ...
            const userConfigPath = _utils_Path__WEBPACK_IMPORTED_MODULE_5__.Path.resolve(options.workDir, _Constants__WEBPACK_IMPORTED_MODULE_10__.USER_CONFIG);
            if (await (0,_utils_FileSystem__WEBPACK_IMPORTED_MODULE_7__.fileExists)(userConfigPath))
        ...
        const configModule = await (0,_utils_Module__WEBPACK_IMPORTED_MODULE_14__.importModule)(configUrl);
